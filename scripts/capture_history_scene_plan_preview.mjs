...
const DEFAULT_PORT = Number(process.env.HISTORY_SCENE_PLAN_PORT ?? "3213");
const HOST = "127.0.0.1";
const OUTPUT_DIR = resolve(ROOT, "docs", "experiments", "artifacts", "exp-22-history-scene-plan-preview");
const OUTPUT_FILE = join(OUTPUT_DIR, "history-scene-plan.png");
...
async function main() {
  ...
  const readServerOutput = collectProcessOutput(server, "history-scene-plan:start");

  try {
    await waitForServer(`${baseUrl}/history`);
    await captureWithChrome(chromePath, `${baseUrl}/history`, OUTPUT_FILE);
    const summary = {
      capturedAt: new Date().toISOString(),
      baseUrl,
      chromePath,
      pageUrl: `${baseUrl}/history`,
