import { mkdir, rm, writeFile, readFile, access } from "node:fs/promises";
import { constants as fsConstants } from "node:fs";
import { dirname, join, resolve } from "node:path";
import { spawn } from "node:child_process";
import { createServer } from "node:net";
import { fileURLToPath } from "node:url";

const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const WEB_APP_DIR = resolve(ROOT, "apps", "web");
const DEFAULT_PORT = Number(process.env.QUICK_ACTION_DELTA_PORT ?? "3216");
const HOST = "127.0.0.1";
const DEFAULT_OUTPUT_DIR = resolve(ROOT, "docs", "experiments", "artifacts", "exp-41-quick-action-visible-delta");
const CHROME_CANDIDATES = [
  process.env.CHROME_PATH,
  "C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe",
  "C:\\Program Files (x86)\\Google\\Chrome\\Application\\chrome.exe",
  "C:\\Program Files\\Microsoft\\Edge\\Application\\msedge.exe",
  "C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe",
].filter(Boolean);

const SAMPLE_FILES = [
  resolve(ROOT, "docs", "sample", "음식사진샘플(규카츠).jpg"),
  resolve(ROOT, "docs", "sample", "음식사진샘플(맥주).jpg"),
];

const QUICK_ACTION_CONFIG = {
  shorterCopy: {
    captureSceneIds: ["s1", "s4"],
    variantPrefix: "shorter-copy",
  },
  highlightPrice: {
    captureSceneIds: ["s2", "s4"],
    variantPrefix: "highlight-price",
  },
};

function parseArgs(argv) {
  const options = {
    lever: "shorterCopy",
    outputDir: DEFAULT_OUTPUT_DIR,
  };
  for (let index = 0; index < argv.length; index += 1) {
    const token = argv[index];
    if (token === "--lever") {
      options.lever = argv[index + 1] ?? options.lever;
      index += 1;
      continue;
    }
    if (token === "--output-dir") {
      options.outputDir = resolve(ROOT, argv[index + 1] ?? ".");
      index += 1;
      continue;
    }
  }
  return options;
}

function delay(ms) {
  return new Promise((resolveDelay) => setTimeout(resolveDelay, ms));
}

async function fileExists(path) {
  try {
    await access(path, fsConstants.F_OK);
    return true;
  } catch {
    return false;
  }
}

function spawnLogged(command, args, options = {}) {
  return spawn(command, args, {
    cwd: ROOT,
    stdio: ["ignore", "pipe", "pipe"],
    shell: false,
    ...options,
  });
}

function quoteWindowsArg(arg) {
  return /[\s"]/u.test(arg) ? `"${arg.replaceAll('"', '\\"')}"` : arg;
}

function spawnNpm(args, options = {}) {
  if (process.platform === "win32") {
    const command = `npm ${args.map(quoteWindowsArg).join(" ")}`;
    return spawnLogged("cmd.exe", ["/d", "/s", "/c", command], options);
  }
  return spawnLogged("npm", args, options);
}

function collectProcessOutput(child, label) {
  let stdout = "";
  let stderr = "";
  child.stdout?.on("data", (chunk) => {
    stdout += chunk.toString();
  });
  child.stderr?.on("data", (chunk) => {
    stderr += chunk.toString();
  });
  child.on("error", (error) => {
    stderr += `\n[${label}] ${error.message}`;
  });
  return () => ({ stdout, stderr });
}

async function runCommand(command, args, label, options = {}) {
  const child = spawnLogged(command, args, options);
  const readOutput = collectProcessOutput(child, label);
  const code = await new Promise((resolveCode, rejectCode) => {
    child.on("close", resolveCode);
    child.on("error", rejectCode);
  });
  const output = readOutput();
  if (code !== 0) {
    throw new Error(`${label} failed (${code})\n${output.stdout}\n${output.stderr}`.trim());
  }
}

async function runNpmCommand(args, label) {
  const child = spawnNpm(args);
  const readOutput = collectProcessOutput(child, label);
  const code = await new Promise((resolveCode, rejectCode) => {
    child.on("close", resolveCode);
    child.on("error", rejectCode);
  });
  const output = readOutput();
  if (code !== 0) {
    throw new Error(`${label} failed (${code})\n${output.stdout}\n${output.stderr}`.trim());
  }
}

async function resolveChromePath() {
  for (const candidate of CHROME_CANDIDATES) {
    if (candidate && (await fileExists(candidate))) {
      return candidate;
    }
  }
  throw new Error("Chrome 또는 Edge 실행 파일을 찾지 못했습니다. CHROME_PATH를 설정해 주세요.");
}

async function waitForServer(url, timeoutMs = 45000) {
  const startedAt = Date.now();
  while (Date.now() - startedAt < timeoutMs) {
    try {
      const response = await fetch(url, { redirect: "manual" });
      if (response.ok) {
        return;
      }
    } catch {}
    await delay(1000);
  }
  throw new Error(`서버가 ${timeoutMs}ms 안에 준비되지 않았습니다: ${url}`);
}

async function waitForGenerated(baseUrl, projectId, timeoutMs = 30000) {
  const startedAt = Date.now();
  while (Date.now() - startedAt < timeoutMs) {
    const response = await fetch(`${baseUrl}/api/projects/${projectId}/status`, { cache: "no-store" });
    if (response.ok) {
      const payload = await response.json();
      if (payload.projectStatus === "generated" || payload.result) {
        return payload;
      }
    }
    await delay(800);
  }
  throw new Error(`생성이 ${timeoutMs}ms 안에 완료되지 않았습니다: ${projectId}`);
}

async function findFreePort(startPort) {
  for (let port = startPort; port < startPort + 100; port += 1) {
    const isFree = await new Promise((resolveIsFree) => {
      const probe = createServer();
      probe.unref();
      probe.once("error", () => resolveIsFree(false));
      probe.listen(port, HOST, () => {
        probe.close(() => resolveIsFree(true));
      });
    });
    if (isFree) {
      return port;
    }
  }
  throw new Error(`사용 가능한 포트를 찾지 못했습니다. 시작 포트: ${startPort}`);
}

async function captureWithChrome(chromePath, url, outputPath) {
  await mkdir(dirname(outputPath), { recursive: true });
  await rm(outputPath, { force: true });
  const args = [
    "--headless=new",
    "--disable-gpu",
    "--hide-scrollbars",
    "--force-device-scale-factor=1",
    "--window-size=1080,1920",
    `--screenshot=${outputPath}`,
    url,
  ];
  await runCommand(chromePath, args, `capture:${url}`);
}

async function stopServer(server) {
  if (!server.pid) {
    return;
  }
  if (process.platform === "win32") {
    try {
      await runCommand("taskkill", ["/pid", String(server.pid), "/t", "/f"], "taskkill");
      return;
    } catch {}
  }
  server.kill("SIGTERM");
  await delay(1200);
  if (!server.killed) {
    server.kill("SIGKILL");
  }
}

async function requestJson(url, init = {}) {
  const response = await fetch(url, {
    cache: "no-store",
    ...init,
  });
  if (!response.ok) {
    const message = await response.text();
    throw new Error(`request failed: ${url}\n${message}`);
  }
  return response.json();
}

async function buildWeb() {
  await runNpmCommand(["run", "build:web"], "build:web");
}

async function uploadSampleAssets(baseUrl, projectId) {
  const formData = new FormData();
  for (const filePath of SAMPLE_FILES) {
    const content = await readFile(filePath);
    formData.append("files", new Blob([content], { type: "image/jpeg" }), filePath.split("\\").at(-1));
  }
  return requestJson(`${baseUrl}/api/projects/${projectId}/assets`, {
    method: "POST",
    body: formData,
  });
}

function normalizeSnapshot(result, scenePlan) {
  const opening = scenePlan.scenes.find((scene) => scene.sceneId === "s1") ?? null;
  const benefit = scenePlan.scenes.find((scene) => scene.sceneId === "s2") ?? null;
  const closing = scenePlan.scenes.at(-1) ?? null;
  return {
    hookText: result.copySet.hookText,
    ctaText: result.copySet.ctaText,
    captionLengths: result.copySet.captions.map((caption) => caption.length),
    openingPrimaryLength: opening?.copy?.primaryText?.length ?? null,
    openingPrimaryText: opening?.copy?.primaryText ?? null,
    benefitPrimaryLength: benefit?.copy?.primaryText?.length ?? null,
    benefitPrimaryText: benefit?.copy?.primaryText ?? null,
    closingPrimaryText: closing?.copy?.primaryText ?? null,
    sceneCount: scenePlan.sceneCount,
  };
}

async function main() {
  const options = parseArgs(process.argv.slice(2));
  const leverConfig = QUICK_ACTION_CONFIG[options.lever];
  if (!leverConfig) {
    throw new Error(`지원하지 않는 quick action입니다: ${options.lever}`);
  }

  await buildWeb();
  const chromePath = await resolveChromePath();
  const port = await findFreePort(DEFAULT_PORT);
  const baseUrl = `http://${HOST}:${port}`;
  const outputDir = options.outputDir;
  await rm(outputDir, { recursive: true, force: true });
  await mkdir(outputDir, { recursive: true });

  const server = spawnNpm(["run", "start", "--", "--hostname", HOST, "--port", String(port)], {
    cwd: WEB_APP_DIR,
    env: {
      ...process.env,
      PORT: String(port),
    },
  });
  const readServerOutput = collectProcessOutput(server, "quick-action-delta:start");

  try {
    await waitForServer(`${baseUrl}/`);

    const created = await requestJson(`${baseUrl}/api/projects`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        businessType: "restaurant",
        regionName: "성수동",
        detailLocation: "서울숲 골목",
        purpose: "promotion",
        style: "b_grade_fun",
        channels: ["instagram"],
      }),
    });

    const uploaded = await uploadSampleAssets(baseUrl, created.projectId);
    await requestJson(`${baseUrl}/api/projects/${created.projectId}/generate`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        assetIds: uploaded.assets.map((asset) => asset.assetId),
        templateId: "T02",
        quickOptions: {
          highlightPrice: false,
          shorterCopy: false,
          emphasizeRegion: false,
        },
      }),
    });
    await waitForGenerated(baseUrl, created.projectId);
    const baselineResult = await requestJson(`${baseUrl}/api/projects/${created.projectId}/result`);
    const baselineScenePlan = await requestJson(`${baseUrl}${baselineResult.scenePlan.url}`);

    for (const sceneId of leverConfig.captureSceneIds) {
      await captureWithChrome(
        chromePath,
        `${baseUrl}/scene-frame/${sceneId}?projectId=${created.projectId}`,
        join(outputDir, `baseline-${sceneId}.png`),
      );
    }

    await requestJson(`${baseUrl}/api/projects/${created.projectId}/regenerate`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        changeSet: {
          [options.lever]: true,
        },
      }),
    });
    await waitForGenerated(baseUrl, created.projectId);
    const changedResult = await requestJson(`${baseUrl}/api/projects/${created.projectId}/result`);
    const changedScenePlan = await requestJson(`${baseUrl}${changedResult.scenePlan.url}`);

    for (const sceneId of leverConfig.captureSceneIds) {
      await captureWithChrome(
        chromePath,
        `${baseUrl}/scene-frame/${sceneId}?projectId=${created.projectId}`,
        join(outputDir, `${leverConfig.variantPrefix}-${sceneId}.png`),
      );
    }

    const baselineSnapshot = normalizeSnapshot(baselineResult, baselineScenePlan);
    const variantSnapshot = normalizeSnapshot(changedResult, changedScenePlan);

    const summary = {
      capturedAt: new Date().toISOString(),
      baseUrl,
      chromePath,
      projectId: created.projectId,
      lever: {
        changeSet: options.lever,
        baselineValue: false,
        variantValue: true,
      },
      baseline: {
        result: baselineResult,
        scenePlanUrl: baselineResult.scenePlan?.url ?? null,
        snapshot: baselineSnapshot,
      },
      variant: {
        result: changedResult,
        scenePlanUrl: changedResult.scenePlan?.url ?? null,
        snapshot: variantSnapshot,
      },
      delta: {
        hookTextLength: (changedResult.copySet.hookText?.length ?? 0) - (baselineResult.copySet.hookText?.length ?? 0),
        openingPrimaryLength: (variantSnapshot.openingPrimaryLength ?? 0) - (baselineSnapshot.openingPrimaryLength ?? 0),
        benefitPrimaryLength: (variantSnapshot.benefitPrimaryLength ?? 0) - (baselineSnapshot.benefitPrimaryLength ?? 0),
        baselineCaptures: leverConfig.captureSceneIds.map((sceneId) => `baseline-${sceneId}.png`),
        variantCaptures: leverConfig.captureSceneIds.map((sceneId) => `${leverConfig.variantPrefix}-${sceneId}.png`),
      },
    };
    await writeFile(join(outputDir, "summary.json"), `${JSON.stringify(summary, null, 2)}\n`, "utf8");
  } finally {
    await stopServer(server);
    const { stdout, stderr } = readServerOutput();
    await writeFile(join(outputDir, "server.log"), `${stdout}\n${stderr}`.trim(), "utf8");
  }
}

main().catch((error) => {
  console.error(error instanceof Error ? error.message : String(error));
  process.exitCode = 1;
});
