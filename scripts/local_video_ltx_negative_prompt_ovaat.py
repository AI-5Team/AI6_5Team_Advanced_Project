...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "experiments" / "artifacts" / "exp-56-local-ltx-video-negative-prompt-ovaat"
BASE_PROMPT = "crispy gyukatsu, bright tabletop lighting, strong steam cloud, realistic food motion, static close-up, minimal camera movement"
BASELINE_NEGATIVE = "worst quality, blurry, jittery, distorted, warped, overexposed, text, watermark"
VARIANT_NEGATIVE = (
    "worst quality, blurry, jittery, distorted, warped, overexposed, text, watermark, "
    "ghosting, duplicated utensils, duplicate bowls, duplicate sauce trays, malformed cutlet, extra food pieces"
)


def run_ltx_variant(output_dir: Path, prompt: str, negative_prompt: str, offline: bool) -> dict[str, object]:
    env = os.environ.copy()
    if offline:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare LTX negative prompt OVAT with fixed bright tabletop food prompt.")
    parser.add_argument("--baseline-negative", default=BASELINE_NEGATIVE)
    parser.add_argument("--variant-negative", default=VARIANT_NEGATIVE)
    parser.add_argument("--offline", action="store_true")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    args = parser.parse_args()

    baseline_dir = args.output_dir / "baseline-default-negative"
    variant_dir = args.output_dir / "variant-stronger-negative"
    baseline_dir.mkdir(parents=True, exist_ok=True)
    variant_dir.mkdir(parents=True, exist_ok=True)

    baseline_summary = run_ltx_variant(baseline_dir, BASE_PROMPT, args.baseline_negative, args.offline)
    variant_summary = run_ltx_variant(variant_dir, BASE_PROMPT, args.variant_negative, args.offline)

    combined = {
        "experiment_id": "EXP-56",
        "lever": "negative_prompt",
        "baseline_negative": args.baseline_negative,
        "variant_negative": args.variant_negative,
