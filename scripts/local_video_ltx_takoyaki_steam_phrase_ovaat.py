...
DEFAULT_IMAGE = REPO_ROOT / "docs" / "sample" / "음식사진샘플(타코야키).jpg"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "experiments" / "artifacts" / "exp-74-local-ltx-takoyaki-steam-phrase-ovaat"
DEFAULT_NEGATIVE_PROMPT = "worst quality, blurry, jittery, distorted, warped, overexposed, text, watermark"
DEFAULT_SEED = 7
BASELINE_PROMPT = "takoyaki snack tray, bright tabletop lighting, strong steam cloud, realistic food motion, static close-up, minimal camera movement"
VARIANT_PROMPT = "takoyaki snack tray, bright tabletop lighting, realistic food motion, static close-up, minimal camera movement"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare takoyaki tray baseline prompt vs no-steam variant on local LTX.")
    parser.add_argument("--offline", action="store_true")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    args = parser.parse_args()

    args.output_dir.mkdir(parents=True, exist_ok=True)
    baseline = run_variant(args.output_dir / "baseline-strong-steam", BASELINE_PROMPT, args.offline)
    variant = run_variant(args.output_dir / "variant-no-steam", VARIANT_PROMPT, args.offline)
    summary = {
        "experiment_id": "EXP-74",
        "validation_type": "takoyaki_steam_phrase_ovaat",
