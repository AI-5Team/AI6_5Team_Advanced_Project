...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "experiments" / "artifacts" / "exp-80-local-ltx-beer-carbonation-phrase-ovaat"
...
VARIANT_PROMPT = "beer bottle and lager glass, bright tabletop lighting, still life beverage shot, subtle carbonation bubbles, crisp foam rim, static close-up, minimal camera movement"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare beer baseline vs carbonation phrase variant on local LTX.")
    parser.add_argument("--offline", action="store_true")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    args = parser.parse_args()

    args.output_dir.mkdir(parents=True, exist_ok=True)
    baseline = run_variant(args.output_dir / "baseline-still-life-beverage-shot", BASELINE_PROMPT, args.offline)
    variant = run_variant(args.output_dir / "variant-carbonation-foam-detail", VARIANT_PROMPT, args.offline)
    summary = {
        "experiment_id": "EXP-80",
        "validation_type": "beer_carbonation_phrase_ovaat",
