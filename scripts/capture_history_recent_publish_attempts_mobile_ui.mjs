...
const DEFAULT_PORT = Number(process.env.HISTORY_RECENT_ATTEMPTS_MOBILE_PORT ?? "3223");
const HOST = "127.0.0.1";
const OUTPUT_DIR = resolve(ROOT, "docs", "experiments", "artifacts", "exp-57-history-recent-publish-attempts-mobile-ui");
const SAMPLE_FILES = [
...
async function main() {
  ...
  const readServerOutput = collectProcessOutput(server, "history-recent-attempts-mobile:start");
  ...
  try {
    ...
    await captureWithChrome(chromePath, screenshotUrl, join(OUTPUT_DIR, "history-recent-publish-attempts-mobile.png"));
