...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "experiments" / "artifacts" / "exp-86-local-ltx-beer-glass-separation-phrase-ovaat"
...
VARIANT_PROMPT = "beer bottle and lager glass, bright tabletop lighting, still life beverage shot, clear separation between bottle and glass, distinct gap, static close-up, minimal camera movement"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare beer baseline vs bottle/glass separation phrase on local LTX.")
    parser.add_argument("--offline", action="store_true")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    args = parser.parse_args()

    args.output_dir.mkdir(parents=True, exist_ok=True)
    baseline = run_variant(args.output_dir / "baseline-cover-bottom", BASELINE_PROMPT, args.offline)
    variant = run_variant(args.output_dir / "variant-glass-separation", VARIANT_PROMPT, args.offline)
    summary = {
        "experiment_id": "EXP-86",
        "validation_type": "beer_glass_separation_phrase_ovaat",
