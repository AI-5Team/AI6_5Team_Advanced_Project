...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "experiments" / "artifacts" / "exp-82-local-ltx-beer-label-negative-prompt-ovaat"
DEFAULT_SEED = 7
BASELINE_PROMPT = "beer bottle and lager glass, bright tabletop lighting, still life beverage shot, static close-up, minimal camera movement"
BASELINE_NEGATIVE_PROMPT = "worst quality, blurry, jittery, distorted, warped, overexposed, text, watermark"
VARIANT_NEGATIVE_PROMPT = "worst quality, blurry, jittery, distorted, warped, overexposed"
...
def run_variant(output_dir: Path, negative_prompt: str, offline: bool) -> dict[str, object]:
    env = os.environ.copy()
    if offline:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare beer baseline negative prompt vs label-safe negative prompt on local LTX.")
    parser.add_argument("--offline", action="store_true")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    args = parser.parse_args()

    args.output_dir.mkdir(parents=True, exist_ok=True)
    baseline = run_variant(args.output_dir / "baseline-text-watermark-off", BASELINE_NEGATIVE_PROMPT, args.offline)
    variant = run_variant(args.output_dir / "variant-label-safe-negative", VARIANT_NEGATIVE_PROMPT, args.offline)
    summary = {
        "experiment_id": "EXP-82",
        "validation_type": "beer_label_negative_prompt_ovaat",
