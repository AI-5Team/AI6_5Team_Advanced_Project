...
const DEFAULT_PORT = Number(process.env.PERMISSION_ERROR_UI_PORT ?? "3219");
const HOST = "127.0.0.1";
const OUTPUT_DIR = resolve(ROOT, "docs", "experiments", "artifacts", "exp-49-instagram-permission-error-fallback-ui");
const SAMPLE_FILES = [
...
async function main() {
  ...
  const readServerOutput = collectProcessOutput(server, "permission-error-ui:start");
  ...
  try {
    ...
    const publishResponse = await requestJson(`${baseUrl}/api/projects/${created.projectId}/publish`, {
      ...
      body: JSON.stringify({
        ...
        thumbnailText: "권한 문제 fallback 확인",
      }),
    });
    const uploadJob = await requestJson(`${baseUrl}/api/upload-jobs/${publishResponse.uploadJobId}`);
    const screenshotUrl = `${baseUrl}/?projectId=${created.projectId}&uploadJobId=${publishResponse.uploadJobId}`;
    await captureWithChrome(chromePath, screenshotUrl, join(OUTPUT_DIR, "instagram-permission-error-home.png"));
    ...
    const summary = {
      ...
      screenshotUrl,
      checks: {
        permissionErrorPersisted: uploadJob.error?.code === "PUBLISH_FAILED",
        assistFallbackTriggered: publishResponse.status === "assist_required",
        screenshotPath: "instagram-permission-error-home.png",
      },
    };
