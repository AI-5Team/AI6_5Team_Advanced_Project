...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "experiments" / "artifacts" / "exp-70-local-ltx-drink-top-bias-generalization"
DEFAULT_IMAGES = [
...
def build_prompt(image_path: Path) -> str:
    label = LABEL_MAP.get(infer_label(image_path), "drink close-up")
    return (
        f"{label}, bright tabletop lighting, realistic drink commercial motion, "
        "static close-up, minimal camera movement"
    )
...
def evaluate_image(image_path: Path, output_dir: Path, offline: bool) -> dict[str, object]:
    ...
    baseline = run_variant(image_path, image_output_dir / "contain_blur", "contain_blur", offline)
    variant = run_variant(image_path, image_output_dir / "cover_top", "cover_top", offline)
    return {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare contain_blur vs cover_top on drink photos for local LTX.")
    ...
    summary = {
        "experiment_id": "EXP-70",
        "validation_type": "drink_top_bias_generalization",
        "seed": DEFAULT_SEED,
        "baseline_prepare_mode": "contain_blur",
        "variant_prepare_mode": "cover_top",
