...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "experiments" / "artifacts" / "exp-65-local-ltx-framing-mode-ovaat"
DEFAULT_IMAGES = [
...
def evaluate_image(image_path: Path, output_dir: Path, offline: bool) -> dict[str, object]:
    ...
    baseline = run_variant(image_path, image_output_dir / "cover_center", "cover_center", offline)
    variant = run_variant(image_path, image_output_dir / "contain_blur", "contain_blur", offline)
    return {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare input framing modes for single-photo LTX food generation.")
    ...
    summary = {
        "experiment_id": "EXP-65",
        "validation_type": "framing_mode_ovaat",
        "seed": DEFAULT_SEED,
        "baseline_prepare_mode": "cover_center",
        "variant_prepare_mode": "contain_blur",
