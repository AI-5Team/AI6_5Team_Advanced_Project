...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "experiments" / "artifacts" / "exp-52-local-ltx-video-texture-emphasis-ovaat"
BASE_PROMPT_SUFFIX = "bright tabletop lighting, strong steam cloud, realistic food motion, static close-up, minimal camera movement"
BASELINE_TEXTURE = "none"
VARIANT_TEXTURE = "detailed crispy breading texture"
...
def build_prompt(texture_phrase: str) -> str:
    segments = ["crispy gyukatsu"]
    if texture_phrase and texture_phrase != "none":
        segments.append(texture_phrase)
    segments.append(BASE_PROMPT_SUFFIX)
    return ", ".join(segments)


def main() -> None:
    parser = argparse.ArgumentParser(description="Compare LTX texture emphasis OVAT with fixed bright tabletop food prompt.")
    parser.add_argument("--baseline-texture", default=BASELINE_TEXTURE)
    parser.add_argument("--variant-texture", default=VARIANT_TEXTURE)
    parser.add_argument("--offline", action="store_true")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    args = parser.parse_args()

    baseline_dir = args.output_dir / "baseline-no-texture"
    variant_dir = args.output_dir / "variant-crispy-texture"
    baseline_dir.mkdir(parents=True, exist_ok=True)
    variant_dir.mkdir(parents=True, exist_ok=True)

    baseline_prompt = build_prompt(args.baseline_texture)
    variant_prompt = build_prompt(args.variant_texture)
    ...
    combined = {
        "experiment_id": "EXP-52",
        "lever": "texture_emphasis",
        "baseline_texture": args.baseline_texture,
        "variant_texture": args.variant_texture,
