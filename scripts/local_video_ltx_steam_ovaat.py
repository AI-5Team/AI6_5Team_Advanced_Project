...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "experiments" / "artifacts" / "exp-48-local-ltx-video-steam-intensity-ovaat"
BASE_PROMPT_SUFFIX = "warm restaurant lighting, realistic food motion, static close-up, minimal camera movement"
BASELINE_STEAM = "gentle steam"
VARIANT_STEAM = "strong steam cloud"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare LTX steam intensity OVAT with fixed static close-up prompt.")
    parser.add_argument("--baseline-steam", default=BASELINE_STEAM)
    parser.add_argument("--variant-steam", default=VARIANT_STEAM)
    parser.add_argument("--offline", action="store_true")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    args = parser.parse_args()

    baseline_dir = args.output_dir / "baseline-gentle-steam"
    variant_dir = args.output_dir / "variant-strong-steam"
    baseline_dir.mkdir(parents=True, exist_ok=True)
    variant_dir.mkdir(parents=True, exist_ok=True)

    baseline_prompt = f"crispy gyukatsu, {args.baseline_steam}, {BASE_PROMPT_SUFFIX}"
    variant_prompt = f"crispy gyukatsu, {args.variant_steam}, {BASE_PROMPT_SUFFIX}"
    ...
    combined = {
        ...
        "lever": "steam_intensity",
        "baseline_steam": args.baseline_steam,
        "variant_steam": args.variant_steam,
