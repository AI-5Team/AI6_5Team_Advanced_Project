...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "experiments" / "artifacts" / "exp-51-local-ltx-video-lighting-phrase-ovaat"
BASE_PROMPT_SUFFIX = "strong steam cloud, realistic food motion, static close-up, minimal camera movement"
BASELINE_LIGHTING = "warm restaurant lighting"
VARIANT_LIGHTING = "bright tabletop lighting"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare LTX lighting phrase OVAT with fixed static close-up food prompt.")
    parser.add_argument("--baseline-lighting", default=BASELINE_LIGHTING)
    parser.add_argument("--variant-lighting", default=VARIANT_LIGHTING)
    parser.add_argument("--offline", action="store_true")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    args = parser.parse_args()

    baseline_dir = args.output_dir / "baseline-warm-lighting"
    variant_dir = args.output_dir / "variant-bright-lighting"
    baseline_dir.mkdir(parents=True, exist_ok=True)
    variant_dir.mkdir(parents=True, exist_ok=True)

    baseline_prompt = f"crispy gyukatsu, {args.baseline_lighting}, {BASE_PROMPT_SUFFIX}"
    variant_prompt = f"crispy gyukatsu, {args.variant_lighting}, {BASE_PROMPT_SUFFIX}"
    ...
    combined = {
        "experiment_id": "EXP-51",
        "lever": "lighting_phrase",
        "baseline_lighting": args.baseline_lighting,
        "variant_lighting": args.variant_lighting,
