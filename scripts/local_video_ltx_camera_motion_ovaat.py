...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "experiments" / "artifacts" / "exp-44-local-ltx-video-camera-motion-ovaat"
BASE_PROMPT_PREFIX = "crispy gyukatsu, gentle steam, warm restaurant lighting, realistic food motion"
BASELINE_MOTION = "slow push-in"
VARIANT_MOTION = "static close-up, minimal camera movement"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare LTX camera motion phrase OVAT with fixed short prompt.")
    ...
    baseline_dir = args.output_dir / "baseline-push-in"
    variant_dir = args.output_dir / "variant-static-close-up"
    baseline_dir.mkdir(parents=True, exist_ok=True)
    variant_dir.mkdir(parents=True, exist_ok=True)

    baseline_prompt = f"{BASE_PROMPT_PREFIX}, {args.baseline_motion}"
    variant_prompt = f"{BASE_PROMPT_PREFIX}, {args.variant_motion}"
