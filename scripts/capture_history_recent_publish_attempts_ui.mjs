...
const DEFAULT_PORT = Number(process.env.HISTORY_RECENT_ATTEMPTS_PORT ?? "3222");
const HOST = "127.0.0.1";
const OUTPUT_DIR = resolve(ROOT, "docs", "experiments", "artifacts", "exp-55-history-recent-publish-attempts-ui");
const SAMPLE_FILES = [
...
async function main() {
  ...
  const readServerOutput = collectProcessOutput(server, "history-recent-attempts:start");
  ...
  try {
    ...
    const firstPublishResponse = await requestJson(`${baseUrl}/api/projects/${created.projectId}/publish`, {
      ...
      body: JSON.stringify({
        ...
        thumbnailText: "최근 시도 1",
      }),
    });
    await waitForUploadJobTerminal(baseUrl, firstPublishResponse.uploadJobId, ["published"]);
    await delay(1200);
    ...
    const secondPublishResponse = await requestJson(`${baseUrl}/api/projects/${created.projectId}/publish`, {
      ...
      body: JSON.stringify({
        ...
        hashtags: result.copySet.hashtags,
        thumbnailText: "최근 시도 2",
      }),
    });
    const recentUploadJobs = await requestJson(`${baseUrl}/api/projects/${created.projectId}/upload-jobs`);

    const screenshotUrl = `${baseUrl}/history?projectId=${created.projectId}`;
    await captureWithChrome(chromePath, screenshotUrl, join(OUTPUT_DIR, "history-recent-publish-attempts.png"));
    ...
    const summary = {
      ...
      screenshotUrl,
      checks: {
        attemptsRecorded: recentUploadJobs.items.length >= 2,
        latestAttemptFirst: recentUploadJobs.items[0]?.uploadJobId === secondPublishResponse.uploadJobId,
        firstAttemptSecond: recentUploadJobs.items[1]?.uploadJobId === firstPublishResponse.uploadJobId,
      },
    };
