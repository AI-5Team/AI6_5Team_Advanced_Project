...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "experiments" / "artifacts" / "exp-84-local-ltx-beer-cover-bottom-bias-ovaat"
...
def run_variant(output_dir: Path, prepare_mode: str, offline: bool) -> dict[str, object]:
    env = os.environ.copy()
    if offline:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare beer cover_center vs cover_bottom on local LTX.")
    parser.add_argument("--offline", action="store_true")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    args = parser.parse_args()

    args.output_dir.mkdir(parents=True, exist_ok=True)
    baseline = run_variant(args.output_dir / "baseline-cover-center", "cover_center", args.offline)
    variant = run_variant(args.output_dir / "variant-cover-bottom", "cover_bottom", args.offline)
    summary = {
        "experiment_id": "EXP-84",
        "validation_type": "beer_cover_bottom_bias_ovaat",
