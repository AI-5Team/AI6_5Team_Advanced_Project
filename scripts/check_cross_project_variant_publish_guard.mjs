...
const DEFAULT_PORT = Number(process.env.CROSS_PROJECT_VARIANT_GUARD_PORT ?? "3226");
const HOST = "127.0.0.1";
const OUTPUT_DIR = resolve(ROOT, "docs", "experiments", "artifacts", "exp-62-cross-project-variant-publish-guard");
const SAMPLE_FILES = [
...
async function createGeneratedProject(baseUrl) {
  ...
  const result = await requestJson(`${baseUrl}/api/projects/${projectId}/result`);
  if (result.status !== 200) {
    throw new Error(`result failed: ${JSON.stringify(result.body)}`);
  }
  return { projectId, result: result.body };
}
...
async function main() {
  ...
  const readServerOutput = collectProcessOutput(server, "cross-project-variant-guard:start");

  try {
    await waitForServer(`${baseUrl}/`);
    const projectA = await createGeneratedProject(baseUrl);
    const projectB = await createGeneratedProject(baseUrl);

    const crossPublish = await requestJson(`${baseUrl}/api/projects/${projectB.projectId}/publish`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        variantId: projectA.result.variantId,
        channel: "youtube_shorts",
        publishMode: "assist",
        captionOverride: projectA.result.copySet.captions[0],
        hashtags: projectA.result.copySet.hashtags,
      }),
    });

    const ownPublish = await requestJson(`${baseUrl}/api/projects/${projectB.projectId}/publish`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        variantId: projectB.result.variantId,
        channel: "youtube_shorts",
        publishMode: "assist",
        captionOverride: projectB.result.copySet.captions[0],
        hashtags: projectB.result.copySet.hashtags,
      }),
    });
    ...
    const summary = {
      ...
      ownPublish,
      checks: {
        differentProjects: projectA.projectId !== projectB.projectId,
        differentVariants: projectA.result.variantId !== projectB.result.variantId,
        crossProjectVariantRejected:
          crossPublish.status === 422 && crossPublish.body?.error?.code === "INVALID_STATE_TRANSITION",
        ownVariantAccepted:
          ownPublish.status === 200 ||
          (ownPublish.status === 202 &&
            (ownPublish.body?.status === "assist_required" || ownPublish.body?.status === "queued")),
      },
    };
