...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "experiments" / "artifacts" / "exp-68-local-ltx-prepare-mode-drink-exception-policy"
DEFAULT_IMAGES = [
...
def choose_baseline_mode(image_path: Path) -> str:
    if infer_food_key(image_path) == "규카츠":
        return "cover_center"
    return "contain_blur"


def choose_variant_mode(image_path: Path) -> str:
...
def evaluate_image(image_path: Path, output_dir: Path, offline: bool) -> dict[str, object]:
    ...
    baseline_mode = choose_baseline_mode(image_path)
    variant_mode = choose_variant_mode(image_path)
    baseline = run_variant(image_path, image_output_dir / f"baseline-{baseline_mode}", baseline_mode, offline)
    variant = run_variant(image_path, image_output_dir / f"variant-{variant_mode}", variant_mode, offline)
    return {
        "image": str(image_path),
        "baseline_prepare_mode": baseline_mode,
        "variant_prepare_mode": variant_mode,
        "baseline": baseline,
        "variant": variant,
        "improvement": {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare shot-type prepare policy vs drink exception policy for local LTX.")
    ...
    summary = {
        "experiment_id": "EXP-68",
        "validation_type": "prepare_mode_drink_exception_policy",
        "seed": DEFAULT_SEED,
        "baseline_policy": {
            "규카츠": "cover_center",
        ...
        "aggregate": {
            ...
            "variant_better_count": len(positive),
