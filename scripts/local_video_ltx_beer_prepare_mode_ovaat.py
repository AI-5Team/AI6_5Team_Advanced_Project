...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "experiments" / "artifacts" / "exp-83-local-ltx-beer-prepare-mode-ovaat"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare beer cover_top vs cover_center on local LTX.")
    parser.add_argument("--offline", action="store_true")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    args = parser.parse_args()

    args.output_dir.mkdir(parents=True, exist_ok=True)
    baseline = run_variant(args.output_dir / "baseline-cover-top", "cover_top", args.offline)
    variant = run_variant(args.output_dir / "variant-cover-center", "cover_center", args.offline)
    summary = {
        "experiment_id": "EXP-83",
        "validation_type": "beer_prepare_mode_ovaat",
