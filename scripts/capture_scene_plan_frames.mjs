...
const DEFAULT_PORT = Number(process.env.SCENE_PLAN_PORT ?? "3211");
const HOST = "127.0.0.1";
const OUTPUT_DIR = resolve(ROOT, "docs", "experiments", "artifacts", "exp-20-scene-plan-web-bridge");
const CAPTURES = [
  { sceneId: "s1", artifact: "exp15-promotion-bgrade-tone", fileName: "promotion-opening.png" },
  { sceneId: "s4", artifact: "exp15-promotion-bgrade-tone", fileName: "promotion-closing.png" },
  { sceneId: "s1", artifact: "exp18-review-region-repeat", fileName: "review-region-opening.png" },
  { sceneId: "s3", artifact: "exp19-review-cta-strength", fileName: "review-cta-closing.png" },
];
...
async function main() {
  ...
  const readServerOutput = collectProcessOutput(server, "scene-plan:start");

  try {
    await waitForServer(`${baseUrl}/scene-frame/s1?artifact=exp15-promotion-bgrade-tone`);
    const artifacts = [];
    for (const capture of CAPTURES) {
      const url = `${baseUrl}/scene-frame/${capture.sceneId}?artifact=${capture.artifact}`;
