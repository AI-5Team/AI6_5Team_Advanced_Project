...
const DEFAULT_PORT = Number(process.env.HISTORY_LATEST_PUBLISH_PORT ?? "3221");
const HOST = "127.0.0.1";
const OUTPUT_DIR = resolve(ROOT, "docs", "experiments", "artifacts", "exp-53-history-latest-publish-attempt");
const SAMPLE_FILES = [
...
async function main() {
  ...
  const readServerOutput = collectProcessOutput(server, "history-latest-publish:start");
  ...
  try {
    ...
    const firstPublishResponse = await requestJson(`${baseUrl}/api/projects/${created.projectId}/publish`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...
        thumbnailText: "첫 시도 auto 성공",
      }),
    });
    const firstUploadJob = await waitForUploadJobTerminal(baseUrl, firstPublishResponse.uploadJobId, ["published"]);

    await delay(1200);

    const secondPublishResponse = await requestJson(`${baseUrl}/api/projects/${created.projectId}/publish`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        variantId: result.variantId,
        channel: "tiktok",
        publishMode: "auto",
        captionOverride: result.copySet.captions[1] ?? result.copySet.captions[0],
        hashtags: result.copySet.hashtags,
        thumbnailText: "두 번째 시도 fallback",
      }),
    });
    const secondUploadJob = await requestJson(`${baseUrl}/api/upload-jobs/${secondPublishResponse.uploadJobId}`);
    ...
    await captureWithChrome(chromePath, screenshotUrl, join(OUTPUT_DIR, "history-latest-publish-status.png"));
    ...
    const summary = {
      ...
      firstUploadJob,
      secondPublishResponse,
      secondUploadJob,
      latestUploadJob,
      screenshotUrl,
      checks: {
        firstPublished: firstUploadJob.status === "published",
        secondAssistRequired: secondUploadJob.status === "assist_required",
        latestMatchesSecondAttempt: latestUploadJob?.uploadJobId === secondPublishResponse.uploadJobId,
        latestChannelIsTiktok: latestUploadJob?.channel === "tiktok",
      },
    };
