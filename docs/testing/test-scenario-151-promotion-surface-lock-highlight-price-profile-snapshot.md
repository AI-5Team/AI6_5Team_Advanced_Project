...
promotion surface-lock highlightPrice=false profile snapshot 및 runtime summary 확인

## 목적

- `prompt-baseline-v1.json`에 추가한 `promotion_surface_lock_highlight_price_off` profile이 실제 snapshot acceptance를 통과하는지 확인합니다.
...
- runtime summary는 `option_match`, `promotion_surface_lock_highlight_price_off`, `timeout_90s_retry1` recommendation을 반환해야 합니다.
...
- 새 option profile은 manifest 기준 acceptance를 통과했고, runtime recommendation metadata에도 정상 연결됐습니다.
