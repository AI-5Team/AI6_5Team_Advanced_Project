# EXP-178 Review Strict Fallback Surface Lock Highlight Price Shorter Copy Off Snapshot

## 목표

- `EXP-177` candidate를 manifest option profile로 연결하고 snapshot acceptance를 확인합니다.
...
- 추가 profile:
  - `review_strict_fallback_surface_lock_highlight_price_shorter_copy_off`
...
- `docs/experiments/artifacts/exp-178-review-strict-fallback-surface-lock-highlight-price-shorter-copy-off-snapshot.json`
...
1. `review / highlightPrice=true / shorterCopy=false / emphasizeRegion=false`는 이제 manifest 안의 실제 option profile입니다.
2. runtime summary도 이 조건에서 새 profile을 정확히 추천합니다.
3. 남은 최상위 비지역 `P2`는 이제 `review / highlightPrice=true / shorterCopy=true` 한 건뿐입니다.
